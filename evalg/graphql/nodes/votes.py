...
class ElectionVoteCounts(graphene.ObjectType):
    ...
    total = graphene.Int(default_value=0, description="total votes")

    self_added_not_reviewed = graphene.Int(
        default_value=0, description="voters not in census, admin review needed"
    )

    admin_added_rejected = graphene.Int(
        default_value=0, description="voter in census, rejected by admin"
    )

    self_added_rejected = graphene.Int(
        default_value=0, description="voter not in census, rejected by admin"
    )

    admin_added_auto_verified = graphene.Int(
        default_value=0, description="voter in census"
    )

    self_added_verified = graphene.Int(
        default_value=0, description="voter not in census, verified by admin"
    )
...
def resolve_election_count_by_id(_, info, **kwargs):
    ...
    election = evalg.database.query.lookup(
        session, evalg.models.election.Election, id=elec_id
    )
...
class AddVote(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        ...
        if not vote_policy.verify_election_is_ongoing():
            logger.error(
                (
                    "Can't add vote, election is not ongoing. user: "
        ...
        if not vote_policy.verify_ballot_content(ballot_data):
            logger.error(
                "Invalid ballot! user %s, voter %s election %s",
                user.person.id,
                voter_id,
                vote_policy.voter.pollbook.election.id,
            )
        ...
        session.commit()
        logger.info(
            "Vote added. user: %s, voter: %s election: %s",
            user.person.id,
            voter_id,
            vote_policy.voter.pollbook.election.id,
        )
        ...
        if current_app.config.get("MAIL_ENABLE"):
            ...
            try:
                send_vote_confirmation_mail_task.delay(
                    user.person.email, election_group_name
                )
            except Exception:
                logger.error(
                    "Could not send email to %s, rabbit down", user.person.email
                )
