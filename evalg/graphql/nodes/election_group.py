...
class ElectionGroup(graphene_sqlalchemy.SQLAlchemyObjectType):
    ...
    persons_with_multiple_verified_voters = graphene.List(lambda: PersonWithVoters)
    ...
    def resolve_latest_election_group_count(self, info):
        ...
        return evalg.proc.election.get_latest_election_group_count(session, self.id)

    @permission_controller
    def resolve_persons_with_multiple_verified_voters(self, info):
        return resolve_persons_with_multiple_verified_voters(self, info, id=self.id)
    ...
    @classmethod
    def get_query_election_groups(cls, info, filter_old=False, only_admin=False):
        admin_roles = cls.get_current_user_admin_roles(info)
        admin_for = [role.group_id for role in admin_roles]
        query = cls.get_query(info).filter(
            evalg.models.election.ElectionGroup.deleted_at.is_(None)
        )
        if filter_old:
            query = query.filter(evalg.models.election.ElectionGroup.is_old == False)
        if only_admin:
            query = query.filter(
                evalg.models.election.ElectionGroup.id.in_(admin_for),
            )
        else:
            query = query.filter(evalg.models.election.ElectionGroup.published)
        return query


def resolve_election_groups(_, info, **kwargs):
    """List all election groups that should be visible to the current user."""

    return ElectionGroup.get_query_election_groups(
        info, kwargs["filter_old"], only_admin=kwargs["only_admin"]
    ).all()
...
def resolve_election_group_by_id(_, info, **kwargs):
    ...
    if election_group.deleted:
        return None
    if (
        kwargs["id"] in admin_for
        or election_group.announced
        or election_group.published
    ):
...
list_election_groups_query = graphene.List(
    ElectionGroup,
    filter_old=graphene.Argument(graphene.Boolean, default_value=False, required=False),
    only_admin=graphene.Argument(graphene.Boolean, default_value=False, required=False),
...
def resolve_persons_with_multiple_verified_voters(_, info, **kwargs):
    ...
    el_grp = session.query(evalg.models.election.ElectionGroup).get(election_group_id)
    ...
    for person, voter in query.all():
        if person.id in person_id2person_with_voters.keys():
            ...
            person_id2person_with_voters[person.id] = PersonWithVoters(person, voter)
...
get_election_template_query = graphene.Field(
    GenericScalar, resolver=resolve_election_template
)
...
def resolve_election_key_meta(_, info, **kwargs):
    ...
    if not can_manage_election_group(session, user, el_grp):
        return None
    key_meta = evalg.proc.group.get_election_key_meta(session, election_group_id)
...
class ElectionGroupCount(graphene_sqlalchemy.SQLAlchemyObjectType):
    ...
    def resolve_initiated_by(self, info):
        ...
        electionGroupCountChanges = (
            db.session.query(ElectionGroupCountVersion)
            .filter(
                ElectionGroupCountVersion.id == election_group_count_id,
            )
            .order_by(ElectionGroupCountVersion.transaction_id)
            .limit(1)
            .all()
        )
        ...
        raise GraphQLError("Could not resolve initiated_by - change record not found")
...
def resolve_election_group_counting_results(_, info, **kwargs):
    ...
    return query.filter(
        evalg.models.election_group_count.ElectionGroupCount.group_id == kwargs["id"]
    )
...
class UpdateBaseSettings(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        ...
        for e in kwargs.get("elections"):
            election = session.query(evalg.models.election.Election).get(e["id"])
...
class PublishElectionGroup(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        session = get_session(info)
        election_group_id = kwargs.get("id")
        election_group = session.query(evalg.models.election.ElectionGroup).get(
            election_group_id
        )
        user = get_current_user(info)
...
class UnpublishElectionGroup(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        ...
        if evalg.proc.election.unpublish_election_group(session, election_group):
    ...
    def mutate(self, info, **kwargs):
        ...
        if election_group.public_key and election_group.published:
            return SetElectionGroupKeyResponse(
                ...
                message=(
                    "The public key cannot be changed if " "an election is published"
                ),
            )

        for election in election_group.elections:
            if election_group.public_key and election.active and election.has_started:
                return SetElectionGroupKeyResponse(
                    success=False,
                    code="cannot-change-key-if-past-start",
                    message=(
                        "The public key cannot be changed if " "an election has started"
                    ),
                )
            else:
                ...
                if any(count.values()):
                    return SetElectionGroupKeyResponse(
                        ...
                        message=(
                            "The public key cannot be changed if "
...
class CountElectionGroup(graphene.Mutation):
    ...
    def mutate(self, info, **kwargs):
        ...
        if not can_manage_election_group(session, user, election_group_counter.group):
        ...
        if evalg.proc.pollbook.get_voters_in_election_group(
            session, election_group_id, self_added=True, reviewed=False
        ).all():
            return CountElectionGroupResponse(
                success=False,
        ...
        if info.context["user"].person:
            election_group_counter.generate_results(
                count, getattr(info.context["user"].person, "display_name", None)
            )
        else:
        ...
        return CountElectionGroupResponse(
            success=True, election_group_count_id=count.id
        )
