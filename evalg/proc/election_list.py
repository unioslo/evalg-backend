...
def add_election_list(session,
    ...
    if election.is_locked:
        logger.info(
            ...
            'The election is locked. election %s', election_id)
        return False
...
def delete_election_list(session, list_id):
    ...
    :return: True if list is deleted, False else.
    """
    election_list = session.query(em.election_list.ElectionList).get(
        list_id
    )
...
def update_election_list(session,
                         name,
                         election_list_id,
                         election_id,
                         description=None,
